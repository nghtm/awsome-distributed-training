HF_V=4.22.1 && git clone -b v${HF_V} https://github.com/huggingface/transformers.git /transformers && \
    pip install --upgrade pip && python -m pip install datasets==2.4.0 python-etcd==0.4.5 transformers==${HF_V} datasets huggingface-hub sacrebleu==2.1.0 evaluate==0.2.2 && \
