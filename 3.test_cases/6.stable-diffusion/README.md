...
Just for simplifaction of testing, we have separate scripts for Single node and Multi node Distributed Data Parallel Testing. 

## Single Node Tests

The `single-node` folder has the Dockerfile and `build.sh` and `run.sh` commands to build the image and run the container.
...
Once you are in the container, run the following to kickstart training:
...
We ran tests on P4de and P5 machines and here is a comparison
